...
var output2_buffer: [1024]u8 = undefined;
...
fn cmdObjCopy(gpa: Allocator, arena: Allocator, args: []const []const u8) !void {
    ...
    switch (out_fmt) {
        ...
        .elf => {
            ...
            try stripElf(arena, &in, &out, elf_hdr, .{
...
fn stripElf(
    ...
    if (options.compress_debug) {
        fatal("zig objcopy: 'compress debug' option unsupported. (use compress_debug_sections a compile time)", .{});
    }

    switch (elf_hdr.is_64) {
        inline else => |is_64| {
            var elf_file = try ElfFile(is_64).parse(allocator, in, elf_hdr);
            ...
            if (filter_complement) |flt| {
                ...
                var dbg_file_writer = dbg_file.writer(&output2_buffer);

                try elf_file.emit(allocator, &dbg_file_writer, in, .{ .section_filter = flt, .compress_debug = options.compress_debug });
                try dbg_file_writer.end();
            }

            const debuglink: ?DebugLink = if (debuglink_path) |path| ElfFileHelper.createDebugLink(path) else null;
            try elf_file.emit(allocator, out, in, .{
...
fn ElfFile(comptime is_64: bool) type {
    ...
    return struct {
        ...
        pub fn parse(gpa: Allocator, in: *File.Reader, header: elf.Header) !Self {
            var arena = std.heap.ArenaAllocator.init(gpa);
            errdefer arena.deinit();
            const allocator = arena.allocator();

            try in.seekTo(0);
            const raw_header: Elf_Ehdr = try in.interface.takeStruct(Elf_Ehdr, .little);
            //{
            //    const bytes_read = try in.preadAll(std.mem.asBytes(&raw_header), 0);
            ...
            const program_segments = blk: {
                ...
                try in.seekTo(header.phoff);
                const program_header = try allocator.alloc(Elf_Phdr, header.phnum);
                for (program_header) |*hdr| {
                    hdr.* = try in.interface.takeStruct(Elf_Phdr, .little);
                }
                //const bytes_read = try in.preadAll(std.mem.sliceAsBytes(program_header), header.phoff);
            ...
            const sections = blk: {
                ...
                try in.seekTo(header.shoff);
                for (raw_section_header) |*hdr| {
                    hdr.* = try in.interface.takeStruct(Elf_Shdr, .little);
                }
            ...
            for (sections, 0..) |*section, idx| {
                ...
                if (need_data or need_strings) {
                    const buffer = try allocator.alignedAlloc(u8, .fromByteUnits(section_memory_align), @intCast(section.section.sh_size));
                    try in.seekTo(section.section.sh_offset);
                    try in.interface.readSliceAll(buffer);
        ...
        fn emit(self: *const Self, gpa: Allocator, out: *File.Writer, in: *File.Reader, options: EmitElfOptions) !void {
            var arena = std.heap.ArenaAllocator.init(gpa);
            defer arena.deinit();
            const allocator = arena.allocator();
            ...
            const debuglink_name: u32 = blk: {
                ...
                const buf = try allocator.alignedAlloc(u8, .fromByteUnits(section_memory_align), new_offset + name.len + 1);
            ...
            if (options.compress_debug) {
                for (self.sections[1..], sections_update[1..]) |section, *update| {
                    ...
                    const compressed_payload = try ElfFileHelper.tryCompressSection(allocator, in, section.section.sh_offset, section.section.sh_size, std.mem.asBytes(&chdr));
            ...
            var cmdbuf = try std.ArrayList(ElfFileHelper.WriteCmd).initCapacity(allocator, 3 + new_shnum);
            defer cmdbuf.deinit(allocator);
            ...
            const updated_section_header = blk: {
                        ...
                        if (payload) |src_data| {
                            ...
                            const dest_data = switch (src.sh_type) {
                                elf.DT_VERSYM => dst_data: {
                                    const data = try allocator.alignedAlloc(u8, .fromByteUnits(section_memory_align), src_data.len);
                ...
                if (options.add_section) |add_section| {
                    ...
                    var section_threaded: std.Io.Threaded = .init_single_threaded;
                    defer section_threaded.deinit();
                    const section_io = section_threaded.io();

                    var section_buffer: [4096]u8 = undefined;
                    const section_io_file: std.Io.File = .{ .handle = section_file.handle };
                    var section_reader = section_io_file.reader(section_io, &section_buffer);
                    const payload = try section_reader.interface.allocRemaining(arena.allocator(), .unlimited);
            ...
            try ElfFileHelper.write(allocator, out, in, cmdbuf.items);
        }
...
const ElfFileHelper = struct {
    ...
    fn write(allocator: Allocator, out: *File.Writer, in: *File.Reader, cmds: []const WriteCmd) !void {
        // consolidate holes between writes:
        //   by coping original padding data from in (by fusing contiguous ranges)
        //   by writing zeroes otherwise
        const zeroes = [1]u8{0} ** 4096;
        var consolidated = try std.ArrayList(WriteCmd).initCapacity(allocator, cmds.len * 2);
        ...
        for (consolidated.items) |cmd| {
            switch (cmd) {
                .write_data => |data| {
                    try out.seekTo(data.out_offset);
                    try out.interface.writeAll(data.data);
                    try out.interface.flush();
                },
                .copy_range => |range| {
                    try out.seekTo(range.out_offset);
                    try in.seekTo(range.in_offset);
                    const copied_bytes = try out.interface.sendFileAll(in, .limited(range.len));
    ...
    fn tryCompressSection(allocator: Allocator, in: *File.Reader, offset: u64, size: u64, prefix: []const u8) !?[]align(8) const u8 {
        _ = allocator;
        _ = in;
        _ = offset;
        _ = size;
        ...
        // TODO: plug to use zstd, or bring back zlib
        // however it is now not very usefull, it is possible to use "compress_debug_sections" at build time.
        return null;
        //if (size < prefix.len) return null;
        //
        //try in.seekTo(offset);
        //var section_reader = std.io.limitedReader(in.reader(), size);
        //
