...
const affine_values: [*]volatile gba.math.I8_8 = @ptrFromInt(gba.mem.oam);
...
pub const AffineTransform = struct {
    ...
    pub const Identity: AffineTransform = (.init(gba.math.I8_8.fromInt(1), .{}, .{}, gba.math.I8_8.fromInt(1)));

    /// Affine transformation matrix components, in row-major order.
    values: [4]gba.math.I8_8 = @splat(.{}),
    ...
    pub fn set(self: AffineTransform, index: u5) void {
        var value_index: usize = 3 + (@as(usize, index) << 2);
    ...
    pub fn scale(x: gba.math.I8_8, y: gba.math.I8_8) AffineTransform {
        return .init(x, .{}, .{}, y);
    }

    /// Return a rotation matrix that will scale an object by the
    /// given amount on each axis. Uses `sin` and `cos`.
    pub fn rotate(angle: i32) AffineTransform {
        const sin_theta = gba.math.sin(angle);
        const cos_theta = gba.math.cos(angle);
        const a = @as(gba.math.I8_8, @bitCast(@as(i16, @intCast(cos_theta.raw() >> 4))));
        const b = @as(gba.math.I8_8, @bitCast(@as(i16, @intCast(sin_theta.raw() >> 4))));
        const zero = gba.math.I8_8.fromInt(0);
        return .init(a, zero.sub(b), b, a);
    }
};
...
pub const Obj = packed struct(u48) {
    ...
    pub fn setSize(self: *align(4) volatile Obj, size: Size) void {
    ...
    pub inline fn setPosition(self: *align(4) volatile Obj, x: u9, y: u8) void {
        self.x_pos = x;
        self.y_pos = y;
    }
};
