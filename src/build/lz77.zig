...
pub fn compress(allocator: std.mem.Allocator, src: []const u8, vram_safe: bool) ![]u8 {
    ...
    // --- header ---
    try out_list.append(0x10); // magic
    try out_list.append(@as(u8, @truncate(src.len & 0xFF)));
    try out_list.append(@as(u8, @truncate((src.len >> 8) & 0xFF)));
    try out_list.append(@as(u8, @truncate((src.len >> 16) & 0xFF)));

    var src_off: usize = 0;
    var buffered_blocks: u8 = 0; // 0-7
    var control_index: usize = undefined; // index of current control byte in out_list

    control_index = try startControlByte(&out_list);
    ...
    while (src_off < src.len) {
        ...
        if (best_len >= 3) {
            ...
            try out_list.append(first_byte);
            try out_list.append(second_byte);
            ...
            // raw literal
            try out_list.append(src[src_off]);
            src_off += 1;
        }
    ...
    for (0..pad_needed) |_| try out_list.append(0);

    return out_list.toOwnedSlice();
}

fn startControlByte(out: *std.ArrayList(u8)) !usize {
    const idx = out.*.items.len;
    try out.*.append(0); // placeholder control byte
    return idx;
}
