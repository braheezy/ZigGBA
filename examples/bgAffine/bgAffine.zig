...
// 0..1: 64x 8bpp tiles from the generated background asset
...
pub export fn main() void {
    ...
    gba.display.memcpyBackgroundTiles8Bpp(0, assets.background.tiles[0..]);
    gba.display.memcpyBackgroundPalette(0, assets.background.palette[0..]);
    ...
    gba.display.bg_ctrl[2] = .initAffine(.{
        .base_screenblock = 5,
        .size = assets.background.background_size,
    });
    const affine_bg_map = gba.display.AffineBackgroundMap.initCtrl(gba.display.bg_ctrl[2]);
    affine_bg_map.copyFrom(assets.background.map[0..]);
