...
pub export fn main() void {
    ...
    // Install the master ISR vector - required for waitVBlank to work
    const isr_main: *volatile u32 = @ptrFromInt(0x03007FFC);
    isr_main.* = @intFromPtr(&interrupt.isr_master);
    ...
    // Copy the metroid sprite graphics (64 tiles, 4bpp) into OBJ VRAM block 0
    gba.mem.memcpy32(tile_ram, &metr.tiles, metr.tiles.len * @sizeOf(u32));
    // Copy bounding-box tiles (64 tiles) right after the main Metroid graphic.
    // These tiles begin at tile index 64 (64 * 32 bytes offset).
    const box_tile_dest: *volatile [512]u32 = @ptrFromInt(gba.mem.vram + 0x10000 + (64 * 32));
    gba.mem.memcpy32(box_tile_dest, &metr.box_tiles, metr.box_tiles.len * @sizeOf(u32));

    // Copy all 32 colors (64 bytes) from metr.pal, filling banks 0 and 1.
    const pal_ram: *volatile [256]u16 = @ptrFromInt(gba.mem.palette + 0x200);
    const pal_src: *const [32]u16 = @ptrCast(&metr.pal);
    // Copy 32 colours (32 half-words) ⇒ 64 bytes.
    gba.mem.memcpy16(pal_ram, pal_src, 32);

    // Set backdrop (BG palette colour 0) to black so transparency shows correctly.
    const bg_pal: *volatile u16 = @ptrFromInt(gba.mem.palette);
    bg_pal.* = 0;

    // Force color 0 (background) to black for both banks to ensure transparency.
    pal_ram[0] = 0; // Bank 0, color 0
    pal_ram[16] = 0; // Bank 1, color 0

    // Make palette index 1 bright red in *both* banks so the border is visible.
    const RED: u16 = 0x7C00; // max red (BGR555)
    pal_ram[1] = RED; // bank 0
    pal_ram[17] = RED; // bank 1
    ...
    const metroid: *align(4) volatile obj.Obj = &obj.objects[0];
    ...
    metroid.* = .{
        ...
        .tile = .{ .index = 0, .block = 0 }, // Use tile 0 from charblock 0
    };
    ...
    shadow_metroid.* = .{
        ...
        .palette = 1,
        .tile = .{ .index = 64, .block = 0 }, // Grey copy + red border
    };
    shadow_metroid.setSize(.@"64x64");
    shadow_metroid.setPosition(@bitCast(x), @bitCast(y));

    // Red border and grey shadow tiles are at index 64; no separate bbox object needed.
